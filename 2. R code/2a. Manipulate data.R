...
setwd("/Users/Brago/Documents/iXperience/2. R code")
...
flights_AA <- filter(flights_clean, carrier = "AA") # gave us an error - why? doesnt work because its one =
...
#what happens if we make label = T? mon, tue, wed etc instead of 1, 2, 3
...
flights_summary <- flights %>% group_by(carrier) %>% summarise(mean_arr_delay = mean(arr_delay, na.rm=T),
                                                               min_arr_delay = min(arr_delay, na.rm=T),
                                                               max_arr_delay = max(arr_delay, na.rm=T))
...
flights_clean %>% drop_na() %>% filter(carrier == "AA") %>% group_by(month) %>% summarize(mean_dep_delay = mean(dep_delay))
